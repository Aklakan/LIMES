...
public class OpExecutorLimes
    extends OpExecutor
{
    public static final Symbol STORAGE_MAP = Symbol.create("storageMap");

    private static final Logger logger = LoggerFactory.getLogger(OpExecutorLimes.class);


    //Map<Node, StorageEntry> storageMap
    @SuppressWarnings("unchecked")
	protected OpExecutorLimes(ExecutionContext execCxt) {
        super(execCxt);
    ...
    protected QueryIterator execute(OpService opService, QueryIterator input) {
        ...
        if(serviceUri.equals("plugin://limes")) {
            logger.debug("Intercepted execution of:\n" + opService);

            Op subOp = opService.getSubOp();
            Query q = OpAsQuery.asQuery(subOp);
...
//        	while(it.hasNext()) { System.out.println("item: " + it.next()); }
            Stream<Binding> stream = Stream.empty();
